    ...
    axios.get(`${process.env.REACT_APP_BASE_URL}/event`).then((n) => {
      ...
      n.data.content.map((event, i) => {
        ...
        if (event.isCustomRange == true) {
          let customDatesList = [];
          let date1 = new Date(event.customRangeEvent.startDate);
          
          let date2 = new Date(event.customRangeEvent.endDate);
          let time_difference = date2.getTime() - date1.getTime();
          let days_difference = time_difference / (1000 * 60 * 60 * 24);
          let eventStartTime = moment(event.customRangeEvent.startTime, ["HH:mm:ss"]).format("hh:mm a")
          let eventEndTime = moment(event.customRangeEvent.endTime, ["HH:mm:ss"]).format("hh:mm a")
         
          for (let i = 1; i < days_difference + 1; i++) {
            var date = new Date(date1)
            date.setDate(date.getDate() + i)
            //checkboxData[i-1] = false
            customDatesList.push({ "eventDate": moment(date).format("YYYY-MM-DD"), "startTime": eventStartTime, "endTime": eventEndTime })
          }
          setTimeout(() => {
            customDatesList.map((e, i) => {
